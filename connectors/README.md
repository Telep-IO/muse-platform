...
v0 implements **paper-send**, **ship-label**, **sumvid**, **shipsignal**, **sign-send**, **fax-send**, **call-send**, **ink-send**, and **domain-send** as callable gateway modules. ShipLabel demo drafts are stored in SQLite. The other modules are in-memory stubs.

These directories are the agent-facing gateway modules only (REST + MCP + OpenAPI). Express fulfillment for call-send, domain-send, fax-send, ink-send, paper-send, and sign-send lives in [`services/`](../services/README.md). Those apps deploy separately, are not npm workspaces, and are not imported here. PaperSend's Express app is `services/paper-send`. ShipLabel's Express app is `services/ship-label`. The gateway calls ShipLabel over `SHIP_LABEL_SERVICE_URL` in test and live.
