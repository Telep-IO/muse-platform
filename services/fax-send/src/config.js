...
// recorded fax-provider authorization reference — mirroring Paper Send's
...
export function readConfig(env = process.env) {
  ...
  if (mode !== 'demo') {
    ...
    // TODO: provider key format validation once the provider is chosen
    // (Phaxio vs Telnyx Fax) in TERMS-DILIGENCE.md.
    if (!env.FAX_PROVIDER) throw new Error('Set FAX_PROVIDER (e.g. phaxio, telnyx).');
    if (!env.FAX_API_KEY) throw new Error('Set FAX_API_KEY.');
  }
  if (mode === 'live' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(env.SUPPORT_EMAIL || ''))
    throw new Error('Live mode requires SUPPORT_EMAIL.');
  if (mode === 'live') {
    for (const name of ['LEGAL_BUSINESS_NAME', 'BUSINESS_ADDRESS', 'FAX_AUTHORIZATION_REFERENCE']) {
  ...
  return {
    ...
    faxProvider: env.FAX_PROVIDER || '',
    faxKey: env.FAX_API_KEY,
    faxSecret: env.FAX_API_SECRET,
    faxWebhookSecret: env.FAX_WEBHOOK_SECRET,
    faxAuthorizationReference: env.FAX_AUTHORIZATION_REFERENCE?.trim() || '',
    supportEmail: env.SUPPORT_EMAIL || '',
    businessName: env.BUSINESS_NAME || 'FaxSend',
