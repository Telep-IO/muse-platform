...
export function openStore(dataDir) {
  const db = new DatabaseSync(join(dataDir, 'faxsend.sqlite'));
  ...
  return {
    ...
    insert: (row) => prepare(`INSERT INTO faxes
      (id, token_hash, state, fax_number, cover_page, pages, amount, currency,
       document_sha256, created_at, updated_at)
      VALUES (?, ?, 'draft', ?, ?, ?, ?, 'usd', ?, ?, ?)`)
      .run(row.id, row.token_hash, row.fax_number, row.cover_page ? 1 : 0,
           row.pages, row.amount, row.document_sha256, row.created_at, row.created_at),
    ...
    transition: (id, from, to, extra = {}) => {
      ...
      const result = prepare(
        `UPDATE faxes SET state = ?, updated_at = ?${sets ? `, ${sets}` : ''}
    ...
    claim: (id, from, to, leaseMs) => {
      const now = Date.now();
      const result = prepare(
        `UPDATE faxes SET state = ?, lease_until = ?, updated_at = ?
    ...
    set: (id, fields) => {
      ...
      prepare(`UPDATE faxes SET ${sets}, updated_at = ? WHERE id = ?`)
        .run(...Object.values(fields), Date.now(), id);
    },
    insertApproval: (faxId, reviewHash, evidence) =>
      prepare('INSERT INTO approvals (fax_id, review_hash, evidence, created_at) VALUES (?, ?, ?, ?)')
        .run(faxId, reviewHash, evidence, Date.now()),
    approval: (faxId) => prepare('SELECT * FROM approvals WHERE fax_id = ?').get(faxId),
    ...
    dueFor: (states, olderThanMs) =>
      prepare(`SELECT * FROM faxes WHERE state IN (${states.map(() => '?').join(',')})
               AND updated_at < ? AND purged = 0`).all(...states, Date.now() - olderThanMs),
    inStates: (states) =>
      prepare(`SELECT * FROM faxes WHERE state IN (${states.map(() => '?').join(',')})
               AND purged = 0`).all(...states),
    unpurgedCount: () => prepare('SELECT COUNT(*) AS n FROM faxes WHERE purged = 0').get().n,
    close: () => db.close(),
  };
}
