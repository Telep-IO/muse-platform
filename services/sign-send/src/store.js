...
function openSqlite(config) {
  ...
  const db = new DatabaseSync(join(config.dataDir, 'signsend.sqlite'));
  ...
  const columns = db.prepare('PRAGMA table_info(envelopes)').all().map(c => c.name);
  for (const [name, ddl] of [['checked_at', 'INTEGER NOT NULL DEFAULT 0'], ['document_sha256', 'TEXT'], ['payment_total', 'INTEGER'], ['payment_tax', 'INTEGER']]) {
    if (!columns.includes(name)) db.exec(`ALTER TABLE envelopes ADD COLUMN ${name} ${ddl}`);
  }
  db.exec(`CREATE TABLE IF NOT EXISTS policies (
    ...
    CREATE TABLE IF NOT EXISTS approvals (
      envelope_id TEXT PRIMARY KEY REFERENCES envelopes(id), accepted_at INTEGER NOT NULL,
...
async function openPostgres(config) {
  ...
  const pool = new Pool({ connectionString: config.databaseUrl, max: 5,
    ...
    statement_timeout: 15000, application_name: 'signsend',
