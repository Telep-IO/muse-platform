...
export function readConfig(env = process.env) {
  ...
  if (mode !== 'demo') {
    ...
    if (!env.LOB_API_KEY?.startsWith(`${prefix}_`)) throw new Error(`Expected a Lob ${prefix} secret key.`);
  ...
  if (mode === 'live') {
    for (const name of ['LEGAL_BUSINESS_NAME', 'BUSINESS_ADDRESS', 'LOB_AUTHORIZATION_REFERENCE']) {
      if (!env[name]?.trim()) throw new Error(`Live mode requires ${name}. See docs/launch-readiness.md.`);
    }
  }
  ...
  return {
    ...
    stripeKey: env.STRIPE_SECRET_KEY, webhookSecret: env.STRIPE_WEBHOOK_SECRET, lobKey: env.LOB_API_KEY,
    supportEmail: env.SUPPORT_EMAIL || '', businessName: env.BUSINESS_NAME || 'PaperSend',
    legalBusinessName: env.LEGAL_BUSINESS_NAME?.trim() || '', businessAddress: env.BUSINESS_ADDRESS?.trim() || '',
    lobAuthorizationReference: env.LOB_AUTHORIZATION_REFERENCE?.trim() || '',
